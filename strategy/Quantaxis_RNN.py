...
from QUANTAXIS import (
    QA_fetch_future_list,#从monggo_DB获取所有期货列表
    QA_fetch_stock_list,#从mongodb 获取所有的股票列表
    QA_fetch_stock_day_adv,#从mongodb 获取股票日线数据
    QA_fetch_future_day_adv,#从mongodb 获取股票日线数据
    )
class RollingProfitStrategy:
    def __init__(self, start_time, stop_time, n=20, ascending=False, stock_init_cash=1000000):
        self.Account = QA.QA_Account(user_cookie= '',portfolio_cookie='')  # 初始化账户
    ...
    # def get_(self, stock_code, the_financial_time):
    #     """
    #     得到高级财务数据
    #     :param stock_code:
    ...
    #     """
    #     financial_report =
    #     print(stock_code,":",type(stock_code),financial_report)
    ...
    #     :return:
    #     """
    #     stock_code_list = QA_fetch_stock_list().code.tolist()
    #     print(stock_code_list[:5])
    #     stock_dict = {'stock': [], 'totalAssets': [], 'price': []}
    #     # the_financial_time = self.get_financial_time()
    #     for stock_code in stock_code_list:
    #         # print(stock_code)
    #         # assets, EPS = self.(stock_code, the_financial_time)
    #         if assets is not None and EPS != 0:
    #             data = QA_fetch_stock_day_adv(stock_code, self.start_time, self.stop_time)
    #             if data is None:
    #                 continue
